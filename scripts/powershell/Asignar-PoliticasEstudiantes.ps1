...
$CarpetaLogs = Join-Path $PSScriptRoot "..\..\resultados\logs"
$CarpetaLogs = [System.IO.Path]::GetFullPath($CarpetaLogs)
...
function Write-Log {
    ...
    # Console Output for Python to capture
    ...
    # File Output
    try { Add-Content -Path $ArchivoLog -Value $Linea -ErrorAction SilentlyContinue } catch {}
}
...
try {
    ...
    try {
        ...
        Write-Log "Sesión detectada y activa."
            ...
            Write-Log "No hay sesión activa o expiró. Intentando conectar..."
            try {
                Connect-MicrosoftTeams -ErrorAction Stop
                Write-Log "Conexión exitosa."
            }
            catch {
                Write-Log "FATAL: No se pudo conectar a Microsoft Teams. Asegúrese de iniciar sesión o tener permisos." -Nivel "ERROR"
                Write-Log "Detalle: $_" -Nivel "ERROR"
                exit 1
            }
        }

        # Validate Policy Package Existence
        try {
            $pkg = Get-CsPolicyPackage -Identity $PackageName -ErrorAction SilentlyContinue
            if (-not $pkg) {
                Write-Log "FATAL: El paquete de políticas '$PackageName' NO EXISTE en este tenant." -Nivel "ERROR"
                exit 1
            }
            Write-Log "Paquete de políticas '$PackageName' verificado."
        }
        catch {}

    }
    catch {
        Write-Log "Error general conectando/validando Teams: $_" -Nivel "ERROR"
        exit 1
    }
