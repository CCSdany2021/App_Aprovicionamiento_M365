<#
.SYNOPSIS
    Asigna el paquete de políticas Education_Teacher a docentes

.DESCRIPTION
    Este script lee un archivo Excel o CSV con correos/códigos de docentes y les asigna
    automáticamente el paquete de políticas 'Education_Teacher' de Microsoft Teams.

    Genera logs detallados y maneja errores de forma robusta.

.PARAMETER ArchivoDocentes
    Ruta al archivo Excel (.xlsx) o CSV (.csv) con la lista de docentes
    Columnas soportadas: UserPrincipalName, Email, UPN, CODIGO
...
.EXAMPLE
    .\Asignar-PoliticasDocentes.ps1 -ArchivoDocentes "C:\archivos\docentes.xlsx"
...
[CmdletBinding()]
param(
    [Parameter(Mandatory=$true, HelpMessage="Ruta al archivo Excel o CSV con docentes")]
    [ValidateScript({Test-Path $_ -PathType Leaf})]
    [string]$ArchivoDocentes,
...
$ArchivoLog = Join-Path $CarpetaLogs "politicas_docentes_$Timestamp.log"
...
function Import-ArchivoDocentes {
    param([string]$Ruta)
...
function Get-UserPrincipalName {
    ...
    $ColumnasPosibles = @('UserPrincipalName', 'UPN', 'Email', 'Mail', 'Correo', 'CODIGO', 'Codigo', 'codigo')
...
Write-Host "  ASIGNACIÓN DE POLÍTICAS TEAMS - DOCENTES" -ForegroundColor Magenta
Write-Host "  Paquete: $PackageName" -ForegroundColor Magenta
Write-Host "=" * 80 -ForegroundColor Magenta
Write-Host ""

Write-Log "Iniciando proceso de asignación de políticas para docentes"
Write-Log "Archivo de entrada: $ArchivoDocentes"
Write-Log "Dominio: $Dominio"

# Verificar módulo ImportExcel para archivos .xlsx
if ($ArchivoDocentes -like "*.xlsx" -or $ArchivoDocentes -like "*.xls") {
...
# =============================================================================
try {
    $Docentes = Import-ArchivoDocentes -Ruta $ArchivoDocentes
}
catch {
...
Write-Host ""
Write-Host "=" * 80 -ForegroundColor Magenta
Write-Host "  PROCESANDO DOCENTES" -ForegroundColor Magenta
Write-Host "=" * 80 -ForegroundColor Magenta
Write-Host ""
...
foreach ($Docente in $Docentes) {
    ...
    # Obtener UPN
    $UPN = Get-UserPrincipalName -Fila $Docente
...
Write-Host ""
Write-Host "=" * 80 -ForegroundColor Magenta
Write-Host "  RESUMEN DE ASIGNACIÓN" -ForegroundColor Magenta
Write-Host "=" * 80 -ForegroundColor Magenta
Write-Host ""
...
Write-Host ""
Write-Host "📝 Log guardado en: $ArchivoLog" -ForegroundColor Magenta
Write-Host ""
...
$ArchivoResultados = Join-Path $CarpetaLogs "resultados_docentes_$Timestamp.csv"
