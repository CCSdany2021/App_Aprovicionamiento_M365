...
class SincronizadorAutomaticoTeams:
    ...
    def __init__(self, departamento_filtro=None, target_upn=None):
        ...
        default_dept = config.DEFAULT_DEPARTMENT if hasattr(config, 'DEFAULT_DEPARTMENT') and config.DEFAULT_DEPARTMENT else f"Estudiante {config.PERIODO_ACTUAL}"
        if not default_dept: default_dept = "Estudiante 2026"
    ...
    def ejecutar(self):
        ...
        try:
            ...
            for idx, est in enumerate(estudiantes):
                ...
                # BUSQUEDA INTELIGENTE DE EQUIPOS (Exacto + Padre + Normalización Ceros)
                equipos_destino = []
                
                # Normalizamos curso estudiante (ej: "09A" -> "9A")
                curso_est_norm = curso_est.lstrip('0')
                
                # 1. Coincidencia Exacta (Directa o Normalizada)
                if curso_est in equipos_por_curso:
                    equipos_destino.extend(equipos_por_curso[curso_est])
                
                # 2. Coincidencia por Nivel/Padre (Soporte Dinámico: 901, 9A, 9-B, 09A, etc.)
                for codigo_curso_team in equipos_por_curso.keys():
                    # Normalizamos código del equipo también (ej: "09" -> "9")
                    codigo_team_norm = codigo_curso_team.lstrip('0')
                    
                    # Chequeo cruzado: 
                    # A. El curso NORMALIZADO del estudiante empieza con el código NORMALIZADO del equipo
                    # B. O la coincidencia exacta normalizada no se atrapó antes
                    
                    # Caso Exacto Normalizado (ej: Team="9A", Est="09A" -> norm="9A"=="9A")
                    if codigo_team_norm == curso_est_norm:
                         if codigo_curso_team != curso_est: # Evitar duplicar si ya entró en el exacto puro
                             nuevos = [t for t in equipos_por_curso[codigo_curso_team] if t not in equipos_destino]
                             equipos_destino.extend(nuevos)
                         continue

                    # Caso Padre/Hijo (ej: Team="9", Est="09A" -> norm prefix "9" in "9A")
                    prefix_len = len(codigo_team_norm)
                    
                    # Debe empezar con el prefijo y ser más largo
                    if len(curso_est_norm) > prefix_len and curso_est_norm.startswith(codigo_team_norm):
                        ...
                        if next_char.isdigit():
                            ...
                            # Regla Alfanumérica: 9A, 9-B -> Válido
                            es_match_valido = True
